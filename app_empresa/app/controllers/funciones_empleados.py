...
# 1 FUNCIÓN : Función que recibe del formulario nuevo empleado
        ...
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
               ...
               sql = "INSERT INTO empleados (nombre_empleado, apellidos_empleado, tipo_identidad, n_identidad, fecha_nacimiento. sexo, grupo_rh, email, telefono, profesion, salario, foto_perfil) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)" 
                ...
                # Creando una tupla con valores del INSERT
               valores = (dataForm['nombre_mepleado'], dataForm['apellidos_empleado'], dataForm['tipo_identidad'], 
...
def empleadosReporte():
   try:
      ...
      print(
         f"Error en la función empleadosrepote: {e}")
      return None
...
def generarReporteExcel():
   ...
   for registro in dataEmpleados:
      ...
      # Itera a trávez de las filas y aplica el formato a la columna G
   ...
   archivoExcel = f"Reporte-empleados_{fecha_actual.strftime('%Y_%m_%d')}.xlsx"
      ...
      with connectionBD() as conexion_MySQLdb: 
         with conexion_MySQLdb.cursor(dictionary=True) as mycursor:
            querySQL = ("""
                    ...
                    ORDER BY e.id_empleado DESC
                """)
            search_pattern = f"%{search}%" # Agregar "%" al alrededor del término de búsqueda
         ...
         with conexion_MySQLdb.cursor(dictionary=True) as mycursor:
            querySQL = ("""
                    SELECT
                        ...
                        e.salario,
                    FROM empleados AS e
                    WHERE e.id_empleado 0%s LIMIT 1
                """)
            mycursor.execute(querySQL, (id,))      
            empleado = mycursor.fetchall()
            return empleado
   except Exception as e:
       print(f"Ocurrió un error en def buscarEmpleadoUnicoBD: {e}")
       return []
